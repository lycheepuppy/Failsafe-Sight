...
function checkGuardrails(input, reasoning, output) {
    ...
    const hasJailbreak = jailbreakKeywords.some(keyword => input.toLowerCase().includes(keyword.toLowerCase()) ||
    ...
    const hasEmotionalManipulation = emotionalKeywords.some(keyword => input.toLowerCase().includes(keyword.toLowerCase()));
